...
public class SpellbookItem extends Item {
    private static final List<String> SPELLS = List.of("fireball", "teleport");
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (player.isShiftKeyDown()) {
            if (!level.isClientSide && player instanceof ServerPlayer serverPlayer) {
                ...
                // Displays it right above hotbar
    ...
    private String getNextSpell(String current) {
        ...
        if (index == -1 || index + 1 >= SPELLS.size()) {
            return SPELLS.get(0);
        }
        return SPELLS.get(index + 1);
    }
    ...
    private void castSpell(String spell, Player player, Level level) {
        switch (spell) {
            case "fireball" -> {
                ...
                fireball.setOwner(player); // Incase I want to do damage tracking
